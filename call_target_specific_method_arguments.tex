...
\label{sec:kwargs}
Keyword arguments (named arguments) in Ruby will serve as a running example in the remainder of this paper, but other constructs~\cite{Zdun05patternsof} such as variable-sized argument lists with a rest argument are amenable to our approach. \autoref{fig:ruby_kwargs} shows how keyword arguments work in Ruby 2.0. Keyword arguments are named arguments. The order in which they are passed in a method call does not matter. Excess keyword arguments are available as a dictionary (``hash'' in Ruby) in the target method's body (\lstinline{**kwargs} in \autoref{lst:kwargs}) or cause an argument error if no such \emph{rest keyword parameter} is defined in the method's signature.

\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,float=htbp,label=fig:ruby_kwargs,caption=Example usage of keyword arguments in Ruby.]
def A.foo(a:, b:)
   a + b
end
...
Although every call site along with its arguments is represented by a single statement in the source code (\eg \autoref{lst:invocation}, \autoref{fig:ruby_kwargs}), the execution environment might decide to pass different arguments for every polymorphic target type.
...
\paragraph{Example}
Consider the case that \autoref{lst:invocation} in \autoref{fig:ruby_kwargs} is executed multiple times, but \lstinline{object} alternates between instances of \lstinline{A}, \lstinline{B}, and \lstinline{C}. Our implementation generates three different argument arrays that are specific to one of the three \lstinline{foo} methods\footnote{For better readability, we use Ruby syntax. Since the arguments array is part of the AST, all objects should AST nodes (fixnum literal nodes, symbol literal nodes and hash literal nodes). Marker nodes are omitted.}.
\begin{Compactitem}
...
In Ruby, keyword arguments can be passed explicitly as named arguments (\autoref{fig:ruby_kwargs}, \autoref{lst:invocation}) and implicitly using an already existing dictionary object (\autoref{fig:ruby_kwargs}, \autoref{lst:invocation2}). The latter case cannot be optimized by expanding dictionaries. Therefore, the original implementation should stay in place for this case.
...
To evaluate our optimization for keyword arguments with call-target-specific method arguments, we ran benchmarks\footnote{\mbox{\url{https://github.com/HPI-SWA-Lab/TargetSpecific-ICOOOLPS}}.} on our optimized JRuby implementation with Truffle on an ordinary HotSpot VM (no Graal). The benchmarks were run on a MacBook Pro with an i7-2720QM CPU and 16~GB RAM.

For every benchmark, we compare two implementations: the original, unoptimized implementation of JRuby (\emph{call-site-specific}); and the optimized call-target-specific implementation. In the latter case, a separate AST subtree is stored with every PIC entry\footnote{This is different from Truffle AST node rewriting because multiple AST subtrees can be stored for a call site at the same time.} (a AST subtree generating the array of arguments passed to the target method is stored for every PIC entry).
